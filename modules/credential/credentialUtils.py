...
def addCredential(data):
        ...
        isOwner = True if 'isOwner' in dict(data).keys() else False
        if hasAccess or isOwner:
                ...
                if isOwner:
                        # To insert in accessPermission table(User who creates the first credential of a project will have both the read and write access for that project)
                        with con.cursor() as cursor:
                                sql = "INSERT INTO accessPermission(userId, canRead, canWrite, description, projectId, isOwner) VALUES ({}, 1, 1, '', '{}', 1)".format(current_identity['userId'], data['projectId'])  
                                cursor.execute(sql)
                                con.commit()
                                cursor.close()
...
def getProjectCredentials(keys):
        ...
        with con.cursor() as cursor:
                ...
                sql = '''SELECT if(JSON_CONTAINS_PATH(c.starredBy,'all',REPLACE(JSON_SEARCH(c.starredBy, 'one', {0}), '"', '')), 1, 0) AS star,
                 ...
                 WHERE c.version = latest AND a.canRead = 1 AND a.userId = {0} AND c.projectId = {1} 
                 GROUP BY c.id, c.version, c.description, a.userId, a.canRead, a.canWrite, a.description, c.starredBy {2}'''.format(current_identity['userId'], keys['pId'], limit)                
                cursor.execute(sql)
        ...
        with con.cursor() as cursor:
                sql = "SELECT * FROM cmsEvents WHERE projectId = {} ORDER BY createdAt DESC".format(keys['pId'])                
                cursor.execute(sql)
        ...
        with con.cursor() as cursor:
                sql = "SELECT a.userId, a.canRead, a.canWrite, a.description AS accessDesc, e.full_name, a.isOwner FROM accessPermission AS a LEFT JOIN employees AS e ON a.userId = e.id WHERE a.projectId = {}".format(keys['pId'])
                cursor.execute(sql)
        ...
        return { 'projectData': projectData, 'eventData': eventData , 'count': countData[0]['count'], 'accessData': accessData, 'hasWriteAccess': hasWriteAccess }
...
def getProjects(keys):
        ...
        with con.cursor() as cursor:
                limit = "LIMIT {}, {}".format((int(keys['page'])*int(keys['limit'])), int(keys['limit']))
                sql = "SELECT p.id, p.project_name, p.created_at, (SELECT count(a.id) FROM accessPermission AS a LEFT JOIN credential AS c ON (c.id = credentialId or c.projectId = a.projectId) LEFT JOIN (Select id, max(version) AS latest FROM credential group by id) AS mx ON mx.id = c.id WHERE c.version = latest AND a.canRead = 1 AND a.userId = {0} AND c.projectId = p.id) AS credentialCount FROM accessPermission AS a LEFT JOIN credential AS c ON (c.id = credentialId or c.projectId = a.projectId) LEFT JOIN project AS p ON p.id = c.projectId OR p.id = a.projectId  WHERE a.canRead = 1 AND a.userId = {0} GROUP BY p.id {1}".format(current_identity['userId'], limit)                
                cursor.execute(sql)
        ...
        # Total number of records
        with con.cursor() as cursor:
                sql = "SELECT COUNT(distinct(p.id)) AS count FROM accessPermission AS a LEFT JOIN credential AS c ON (c.id = credentialId or c.projectId = a.projectId) LEFT JOIN project AS p ON p.id = c.projectId WHERE a.canRead = 1 AND a.userId = {} GROUP BY a.userId".format(current_identity['userId'])                
                cursor.execute(sql)
                countData = cursor.fetchall()
                cursor.close()  
        con.close()
...
def getCredentialDetails(credentialId):
        ...
        with con.cursor() as cursor:
                sql = "SELECT * FROM cmsEvents WHERE credentialId = '{}' ORDER BY createdAt DESC".format(credentialId)                
                cursor.execute(sql)
                eventData = cursor.fetchall()
                cursor.close()
        ...
        with con.cursor() as cursor:
                sql = "SELECT a.userId, a.canRead, a.canWrite, a.description AS accessDesc, a.projectId, e.full_name, a.isOwner FROM accessPermission AS a LEFT JOIN employees AS e ON a.userId = e.id WHERE projectId = (SELECT projectId FROM credential WHERE id = '{0}' GROUP BY projectId);".format(credentialId)                
                cursor.execute(sql)
